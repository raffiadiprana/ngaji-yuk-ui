import { BadRequest, MethodNotAllowed, Forbidden, GeneralError } from '@feathersjs/errors';
...
function transformItems(items, fieldNames, transformer) {
  (Array.isArray(items) ? items : [items]).forEach((item) => {
    fieldNames.forEach((fieldName) => {
      transformer(item, fieldName, _get3(item, fieldName));
    });
  });
}
...
function discard(...fieldNames) {
  return (context) => {
    ...
    const items = getItems(context);
    const convert = (item) => _omit(item, fieldNames);
...
function fastJoin(resolvers, query) {
  return (context) => {
    ...
    if (params._populate || params._graphql) return context;
      ...
      (result) => Promise.all(
        joins.map(({ args = [], resolver, joins: joins2 }) => {
          return Promise.resolve(resolver(...args)(result, context)).then((addedResults) => {
            if (!addedResults || !joins2) return context;
...
.....hook called. type ${context.type} method ${context.method} resolved skipHookWhen ${skipHookWhen}`
    ...
    if (context.params.$populate) return context;
  ...
  return (context) => {
    ...
    transformItems(getItems(context), fieldNames, (item, fieldName, value) => {
      if (value !== void 0) {
        ...
        _set3(item, fieldName, value ? value.toLowerCase() : value);
      }
    });
    return context;
  };
...
function mongoKeys(ObjectId, keyFields) {
  ...
  return (context) => {
    ...
    const query = context.params.query || {};
    traverser(query).forEach(function(node) {
...
function populateItem(options, context, item, includeSchema, depth) {
  ...
  return Promise.all(
    include.map((childSchema) => {
      ...
      if (!query && !select && (!parentField || _get3(item, parentField) === void 0)) {
        return void 0;
      }
    ...
    children.forEach((child) => {
      if (child) {
        _set3(item, child.nameAs, child.items);
      }
    });
    return item;
  });
...
function populateAddChild(options, context, parentItem, childSchema, depth) {
  ...
  return Promise.resolve().then(() => select ? select(context, parentItem, depth) : {}).then((selectQuery) => {
    let sqlQuery = {};
    if (parentField) {
      const parentVal = _get3(parentItem, parentField);
    ...
    function serializeItem(item, schema2) {
      ...
      if (only) {
        ...
        only.concat("_include", "_elapsed", item._include || []).forEach((key) => {
          const value = _get3(item, key);
          if (value !== void 0) {
            _set3(newItem, key, value);
          }
        });
        item = newItem;
      }
...
var debug3 = makeDebug("feathers-hooks-common/setField");
function setField({
  as,
  from,
  ...
  return (context) => {
    ...
    const value = _get3(context, from);
    if (value === void 0) {
  ...
  return (context) => {
    ...
    if (context.params && context.params.provider === "rest") {
      ...
      if (typeof value === "string" && value[0] !== ":") {
        _set3(context.params, fieldName, value);
      }
    }
  };
}
...
function callingParams({
  ...
  return (context) => {
    ...
    allPropNames.forEach((name) => {
      if (name) {
        const value = _get3(context.params, name);
        if (value !== void 0) {
          _set3(newParams, name, value);
        }
      }
    });
...
export { actOnDefault, actOnDispatch, alterItems, cache, callingParams, callingParamsDefaults, checkContext, checkContextIf, combine, dePopulate, debug, disablePagination, disallow, discard, discardQuery, every, fastJoin, fgraphql, getItems, hookTypes, iff, iffElse, isNot, isProvider, keep, keepInArray, keepQuery, keepQueryInArray, lowerCase, makeCallingParams, methodNames, mongoKeys, paramsForServer, paramsFromClient, populate, preventChanges, replaceItems, required, runHook, runParallel, sequelizeConvert, serialize, setField, setNow, setSlug, sifter, softDelete, some, stashBefore, traverse3 as traverse, unless, validate, validateSchema, iff as when };
