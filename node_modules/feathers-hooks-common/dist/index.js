...
var traverser = require('neotraverse/legacy');
...
function replaceItem(item, fields) {
  ...
  fields.forEach((field) => {
    if (!_has__default.default(item, field)) return;
...
function required(...fieldNames) {
  return (context) => {
    ...
    (Array.isArray(items) ? items : [items]).forEach((item) => {
      fieldNames.forEach((name) => {
        if (!_has__default.default(item, name)) throw new errors.BadRequest(`Field ${name} does not exist. (required)`);
